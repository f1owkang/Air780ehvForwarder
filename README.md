...
|------|------|
| `帮助` / `help` | 分组指令菜单 |
| `状态` / `status` | 信号、网络、内存、规则数、短信缓存 |
| `信号` / `signal` | 信号强度（RSRP/CSQ + 优/良/一般/差） |
| `设备` / `device` | IMEI/IMSI/ICCID/本机号码 |
| `定位` / `位置` / `location` | 基站定位，返回地图链接 |
| `流量` / `查流量` / `traffic` | 向运营商发送流量查询短信 |
| `时间` / `time` | 设备时间与同步状态 |
| `规则` / `rules` | 列出转发规则（目标地址打码显示） |
| `短信 [N]` / `sms [N]` | 最近收到的 N 条短信（默认 5，上限 20，fskv 持久化） |
| `发短信 号码 内容` | 通过设备 SIM 卡发送短信 |
| `测试` / `test` | 触发一次测试转发 |
| `重载规则` / `reload` | 重新加载转发规则 |
| `飞行模式` / `flymode` | 开关一次飞行模式（网络自愈） |
| `重启` / `reboot` | 重启设备（需再发 `确认` 二次确认） |
...
**Markdown 排版**（可选）：`config.lua` 中设置 `QQBOT_MARKDOWN = true` 后，菜单/状态等回复以 markdown 格式发送（需机器人具备 markdown 消息权限）；无权限时自动回退纯文本。
