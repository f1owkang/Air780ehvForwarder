...
| 🛡️ 容错机制 | 转发失败备用通知队列 + fskv 断电恢复；连续失败自动开关飞行模式恢复网络；供电电压监控低电告警 |
...
3. **烧录**：Luatools 直接选择 `script` 文件夹导入全部脚本，固件选择 `LuatOS-SoC_V2050_Air780EHV_101.soc` 下载；下载失败请先**关闭电脑蓝牙**（蓝牙虚拟串口会干扰 Luatools 下载）；
...
设备直连 Qbot，无需任何服务器。在 QQ 里私聊或在群里 @它 即可下发指令；回复均为被动消息，不受主动消息频控限制。长连接心跳流量约 30–50MB/月；流量紧张可开启**夜间省流**（`QQBOT_NIGHT_SAVE = true`）：每天 23:30~07:00（可配，支持跨零点）wss 自动休眠，时段结束自动恢复，休眠期间短信转发不受影响（走 REST 推送），仅夜间指令暂时不可用。
...
3. 烧录后在 QQ 给机器人发一条消息，机器人会回复**欢迎消息**——内含你的 openid 和配置方法（每个 openid 最多回复 3 次，防止刷回复）；
4. 填入白名单并重新烧录：
...
|------|------|
| `帮助` / `help` | 分组指令菜单 |
| `状态` / `status` | **一屏全览**：信号（RSRP/CSQ/RSRQ/SNR）、网络、电源电压、时间、定位（地图链接）、设备标识（打码）、内存、规则数、短信缓存 |
| `规则` / `rules` | 转发规则列表（目标打码显示） |
| `流量` / `查流量` / `traffic` | 向运营商发送流量查询短信 |
| `短信 [N]` / `sms [N]` | 最近收到的 N 条短信（默认 5，上限 20，fskv 持久化） |
| `发短信 号码 内容` | 通过设备 SIM 卡发送短信 |
| `测试` / `test` | 触发一次测试转发 |
| `重载规则` / `reload` | 重新加载转发规则 |
| `飞行模式` / `flymode` | 开关一次飞行模式（网络自愈） |
| `重启` / `reboot` | 重启设备（需再发 `确认` 二次确认，`取消` 可撤销） |

**原生菜单**：Qbot 连接成功后自动在**单聊窗口底部注册官方菜单**（状态 / 短信 / 规则 / 更多▾），点击即把指令填入输入框发送，无需打字；`QQBOT_MENU = false` 可关闭。

**生命周期欢迎**：用户添加机器人为好友时自动私信欢迎（含 openid 与配置方法）；机器人被拉入群时自动发送群问候（说明 @指令 与白名单要求）。欢迎额度与防刷限制共用（每 openid 3 次）。

**管理端可选配置**（[q.qq.com](https://q.qq.com/) 控制台）：在「指令配置」中把常用指令登记为官方 `/` 斜杠指令（上限 24 个），输入 `/` 即可拉起官方指令面板，体验更原生。

**输入体验：**

- 支持 `/` 前缀（`/状态`、`/help`），全角字符自动转半角，多余空格自动忽略；
- 群里只 @机器人 不带内容 = 打开指令菜单；
- 未知指令给出前缀建议（如发送 `zt` 会提示是否想找 `状态`）并附菜单；
- `重启` 等危险操作需在 60 秒内发送 `确认` 才会执行，发送 `取消` 可撤销。

消息排版原则：emoji 保持克制（仅 ✅ 成功 / ❌ 失败 / ⚠️ 危险三类语义符号），信息展示靠 markdown 排版——加粗标签、分组列表、引用提示行、代码框与链接。

**Markdown 排版与按钮**：markdown 排版默认开启（`QQBOT_MARKDOWN = true`），菜单/状态等回复以 markdown 格式发送（标题加粗、配置行代码框，需机器人具备 markdown 消息权限）；无权限时自动回退纯文本并去除样式符号。再设 `QQBOT_BUTTONS = true` 后，**指令菜单附带按钮键盘**（8 个常用指令两行排列），**重启确认消息附带「确认/取消」按钮**——按钮为回调式（点击直达后台执行、不在会话留下指令文本，非白名单点击会收到欢迎引导）；指令权限由 openid 白名单把关。按钮同样无权限时自动逐级降级（md+按钮 → 纯 markdown → 纯文本）。
...
- **消息 URL 白名单**：若机器人在管理端启用了 URL 白名单，消息中的链接（如定位地图 `apis.map.qq.com`）需报备域名后才会显示，未报备时链接可能被客户端屏蔽；
...
4. *下载/烧录失败*：**关闭电脑蓝牙**（蓝牙虚拟串口会干扰 Luatools 下载），重插 USB 或更换数据线，确认驱动正常。
